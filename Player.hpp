...
private:
    sf::Sprite sprite;
    float velocity = 0.f;
    sf::Music jumpSound; // ✅ chỉ cần Music
...
public:
    ...
    void loadSound(const std::string& jumpPath) {
        if (!jumpSound.openFromFile(jumpPath)) {
            std::cerr << "⚠️ Không thể mở âm thanh MP3: " << jumpPath << std::endl;
        }
    }

    void jump() {
        velocity = -300.f;
        jumpSound.stop(); // đảm bảo phát lại từ đầu
        jumpSound.play();
    }
    ...
    void reset() { // ✅ KHÔNG có Player:: ở đây
           sprite.setPosition(200.f, 300.f);
           velocity = 0.f;
       }
